...
eigenvecs <- read.table("data/plink_pca/all_nprcs_wgs_pca.eigenvec")
eigenvals <- read.table("data/plink_pca/all_nprcs_wgs_pca.eigenval")
...
p_go <- ggplot(data = eigenvecs, aes(x = V3, y = V4, color = geographic_origin)) + geom_point() +
  ...
  xlab("PC 1 (20.5%)") + ylab("PC 2 (6.2%)") + theme_minimal() + theme(
...
ggsave("data/plink_pca/plink_pca.png", plot = final_plot_annotated, dpi = 450, width = 16, height = 9)
